...
until [ -n "$ES_NODE1" ]; do ES_NODE=$(cat .es_node | grep '^:' | grep -oP '(node...)'); done
ES_PID1=$!
until [ -n "$(cat .es_log* | grep YELLOW)" ]; do sleep 1; done
echo "elasticsearch1 should be running now on node $ES_NODE1:$ES_PORT (connected to process $ES_PID1)" &

prun -o .es_log -v -np 1 ESPORT=$ES_PORT $ES_BIN </dev/null 2> .es_node &
echo "waiting for elasticsearch2 to set up..."
until [ -n "$ES_NODE2" ]; do ES_NODE=$(cat .es_node | grep '^:' | grep -oP '(node...)'); done
ES_PID2=$!
until [ -n "$(cat .es_log* | grep YELLOW)" ]; do sleep 1; done
echo "elasticsearch2 should be running now on node $ES_NODE2:$ES_PORT (connected to process $ES_PID2)" &
