...
def get_candidate_entities(input, st):
    ...
    with open('Output2/sample-output' +input[0][12:-1]+ '.csv', 'w', newline='') as myfile:
...
try: TOP_ID = TopMatches(queries)
    except:
        pdb.set_trace()



def TopMatches(queries)	:
    for index in range(len(queries)):
        ...
        TOP_ID=[]
        for i in sorted(ids, key=get_best, reverse=True)[:1]:TOP_ID.append([labels[i],i])
        print('Best matches:')
        for i in sorted(ids, key=get_best, reverse=True)[:5]:
